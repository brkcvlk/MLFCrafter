...
class CleanerCrafter:
    ...
    def run(self, context: dict) -> dict:
            ...
            if df_cleaned[col].isnull().sum() == 0:
                ...
                if self.strategy == "auto":
                    if df_cleaned[col].dtype == "object":
                        logger.debug(f"Auto-filling categorical column '{col}' with '{self.str_fill}'")
                        df_cleaned[col] = df_cleaned[col].fillna(self.str_fill)
                    else:
                        logger.debug(f"Auto-filling numerical column '{col}' with {self.int_fill}")
                        df_cleaned[col] = df_cleaned[col].fillna(self.int_fill)

                elif self.strategy == "mean":
                    if np.issubdtype(df_cleaned[col].dtype, np.number):
                        mean_val = df_cleaned[col].mean()
                        logger.debug(f"Filling column '{col}' with mean value: {mean_val:.4f}")
                        df_cleaned[col] = df_cleaned[col].fillna(mean_val)

                elif self.strategy == "median":
                    if np.issubdtype(df_cleaned[col].dtype, np.number):
                        median_val = df_cleaned[col].median()
                        logger.debug(f"Filling column '{col}' with median value: {median_val:.4f}")
                    ...
                    logger.debug(f"Dropped {dropped_rows} rows containing missing values")

                elif self.strategy == "constant":
                    if df_cleaned[col].dtype == "object":
                        logger.debug(f"Filling categorical column '{col}' with constant '{self.str_fill}'")
                        df_cleaned[col] = df_cleaned[col].fillna(self.str_fill)
                    else:
                        logger.debug(f"Filling numerical column '{col}' with constant {self.int_fill}")
                        df_cleaned[col] = df_cleaned[col].fillna(self.int_fill)
        ...
        logger.info(f"Cleaning completed. Remaining missing values: {remaining_missing}")
